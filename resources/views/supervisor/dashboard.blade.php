<x-supervisor-layout>
    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div class="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm flex items-center">
                <div class="p-3 bg-yellow-100 dark:bg-yellow-900 rounded-full mr-4">
                    <ion-icon name="person-add-outline" class="text-2xl text-yellow-600 dark:text-yellow-400"></ion-icon>
                </div>
                <div>
            ...
            <div class="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm flex items-center">
                <div class="p-3 bg-blue-100 dark:bg-blue-900 rounded-full mr-4">
                    <ion-icon name="document-attach-outline" class="text-2xl text-blue-600 dark:text-blue-400"></ion-icon>
                </div>
                <div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
            <h3 class="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4 p-6 border-b dark:border-gray-700">Tugas Persetujuan Mendesak</h3>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Tipe Pengajuan</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Tanggal Diajukan</th>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900 dark:text-white">{{ $tugas->karyawan }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ $tugas->url }}" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700">Tinjau</a>
                            </td>
                        </tr>
                        @empty
                        <tr>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
             <h3 class="text-xl font-bold text-gray-800 dark:text-gray-100 mb-4">Anggota Tim Saya ({{ $teamMembers->count() }} Orang)</h3>
             <div class="space-y-4">
                
                @forelse ($teamMembers as $member)
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        <img class="h-10 w-10 rounded-full object-cover mr-3" src="https://i.pravatar.cc/150?u={{ $member->email }}" alt="Foto">
                        <div>
                            <p class="font-semibold text-gray-900 dark:text-white">{{ $member->name }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ $member->position->title ?? 'Belum ada posisi' }}</p>
                        </div>
                    </div>
                    <a href="{{-- route('supervisor.tim.detail', $member->id) --}}" class="text-indigo-600 dark:text-indigo-400 hover:underline font-semibold text-sm">Lihat Profil</a>
                </div>
                @empty
