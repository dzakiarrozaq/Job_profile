<x-supervisor-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{route('supervisor.job-profile.create')}}" class="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg hover:bg-indigo-700">
    ...
    <div class="max-w-7xl mx-auto space-y-6">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
            <div class="p-6 border-b dark:border-gray-700">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Kelola semua standar kompetensi untuk setiap jabatan.</p>
            </div>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $profile->position->department->name ?? 'Departemen Dihapus' }}</p>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ $profile->creator->name ?? 'N/A' }}</td>
                            ...
                            <td class="px-6 py-4 text-right flex justify-end space-x-2">
                                ...
                                <form action="{{ route('supervisor.job-profile.destroy', $profile->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus Job Profile ini? Tindakan ini tidak dapat dibatalkan.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="px-3 py-1 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700">
                                        Hapus
                                    </button>
                                </form>
                            </td>
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-4 text-center text-gray-500 dark:text-gray-400">
                                Belum ada Job Profile yang dibuat.
                            </td>
                        </tr>
                        @endforelse
