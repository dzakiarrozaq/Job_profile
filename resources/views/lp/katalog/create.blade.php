<x-lp-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Tambah Katalog Baru</h2>
    </x-slot>

    <div class="py-8 max-w-2xl mx-auto px-4">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm">
            <form action="{{ route('lp.katalog.store') }}" method="POST">
                @csrf
                
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Judul Pelatihan</label>
                    <input type="text" name="title" required class="w-full rounded-md border-gray-300 dark:bg-gray-700 dark:text-white mt-1">
                </div>

                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Provider</label>
                        <input type="text" name="provider" required class="w-full rounded-md border-gray-300 dark:bg-gray-700 dark:text-white mt-1">
                    </div>
                    <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="cost" required class="w-full rounded-md border-gray-300 dark:bg-gray-700 dark:text-white mt-1">
                </div>

                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Deskripsi</label>
                    <textarea name="description" rows="3" class="w-full rounded-md border-gray-300 dark:bg-gray-700 dark:text-white mt-1"></textarea>
                </div>
